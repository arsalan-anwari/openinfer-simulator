...
pub fn round_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_round(OpMode::Normal, attrs, inputs, output)
}

pub fn round_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_round(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_round(
    ...
    crate::vk_trace!(
        "dispatch_round mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("round expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("round requires an output tensor"))?;
    ...
    if !op_supports_dtype(OpKind::Round, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::Round, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan round target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Round.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<RoundPush>() as u32,
    };

...
pub fn trunc_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_trunc(OpMode::Normal, attrs, inputs, output)
}

pub fn trunc_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_trunc(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_trunc(
    ...
    crate::vk_trace!(
        "dispatch_trunc mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("trunc expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("trunc requires an output tensor"))?;
    ...
    if !op_supports_dtype(OpKind::Trunc, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::Trunc, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan trunc target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Trunc.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<TruncPush>() as u32,
    };

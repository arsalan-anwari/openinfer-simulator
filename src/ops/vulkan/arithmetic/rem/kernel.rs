...
pub fn rem_normal_dispatch(
    _attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_rem(OpMode::Normal, _attrs, inputs, output)
}

pub fn rem_inplace_dispatch(
    _attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_rem(OpMode::Inplace, _attrs, inputs, output)
}
...
fn dispatch_rem(
    ...
    crate::vk_trace!(
        "dispatch_rem mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 2 {
        return Err(anyhow!("rem expects 2 inputs, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("rem requires an output tensor"))?;
    ...
    let target = match target_name(OpKind::Rem, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan rem target={}", target);

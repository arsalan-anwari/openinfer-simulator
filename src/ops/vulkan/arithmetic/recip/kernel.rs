...
pub fn recip_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_recip(OpMode::Normal, attrs, inputs, output)
}

pub fn recip_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_recip(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_recip(
    ...
    crate::vk_trace!(
        "dispatch_recip mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("recip expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("recip requires an output tensor"))?;
    ...
    if !op_supports_dtype(OpKind::Recip, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let scalar_attrs = collect_scalar_attr_bits(OpKind::Recip, input_dtype, attrs)?;
    ...
    let target = match target_name(OpKind::Recip, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan recip target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Recip.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<RecipPush>() as u32,
    };

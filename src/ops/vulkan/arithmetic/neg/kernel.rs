...
pub fn neg_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_neg(OpMode::Normal, attrs, inputs, output)
}

pub fn neg_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_neg(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_neg(
    ...
    crate::vk_trace!(
        "dispatch_neg mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("neg expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("neg requires an output tensor"))?;
    ...
    let target = match target_name(OpKind::Neg, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan neg target={}", target);

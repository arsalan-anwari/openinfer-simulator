...
pub fn div_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_div(OpMode::Normal, attrs, inputs, output)
}

pub fn div_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_div(OpMode::Inplace, attrs, inputs, output)
}

fn default_mask_bits(dtype: DType) -> ScalarAttrBits {
    let mut bits = ScalarAttrBits::default();
    if dtype.is_float() {
        let f32_nan = f32::NAN.to_bits();
        let f64_nan = f64::NAN.to_bits();
        bits.f32_bits = f32_nan;
        bits.f64_bits = f64_nan;
    }
    bits
}
...
fn dispatch_div(
    ...
    crate::vk_trace!(
        "dispatch_div mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 2 {
        return Err(anyhow!("div expects 2 inputs, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("div requires an output tensor"))?;
    ...
    let inputs_used: &[TensorValue] = inputs;
    ...
    if !op_supports_dtype(OpKind::Div, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let scalar_attrs = collect_scalar_attr_bits(OpKind::Div, input_dtype, attrs)?;
    let mask_bits = scalar_attrs
        .get("div_by_zero_mask")
        .copied()
        .unwrap_or_else(|| default_mask_bits(input_dtype));

    let mut io_buffers = prepare_staging_io(mode, inputs_used, output, output_dtype)?;
    ...
    let target = match target_name(OpKind::Div, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan div target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Div.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<DivPush>() as u32,
    };

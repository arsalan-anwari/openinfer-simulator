...
pub fn is_neg_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_is_neg(OpMode::Normal, attrs, inputs, output)
}
...
fn dispatch_is_neg(
    ...
    crate::vk_trace!(
        "dispatch_is_neg mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("is_neg expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("is_neg requires an output tensor"))?;
    let input_dtype = inputs[0].dtype();
    let output_dtype = DType::Bool;
    if output.dtype() != DType::Bool {
        return Err(anyhow!(
            "is_neg requires bool output dtype, got {:?}",
            output.dtype()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::IsNeg, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::IsNeg, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan is_neg target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::IsNeg.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<IsNegPush>() as u32,
    };

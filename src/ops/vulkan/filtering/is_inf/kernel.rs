...
pub fn is_inf_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_is_inf(OpMode::Normal, attrs, inputs, output)
}
...
fn dispatch_is_inf(
    ...
    crate::vk_trace!(
        "dispatch_is_inf mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("is_inf expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("is_inf requires an output tensor"))?;
    let input_dtype = inputs[0].dtype();
    let output_dtype = DType::Bool;
    if output.dtype() != DType::Bool {
        return Err(anyhow!(
            "is_inf requires bool output dtype, got {:?}",
            output.dtype()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::IsInf, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::IsInf, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan is_inf target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::IsInf.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<IsInfPush>() as u32,
    };

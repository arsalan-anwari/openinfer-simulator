...
use crate::ops::vulkan::op_helpers::{
    build_desc_bytes, build_output_desc, dispatch_with_standard_bindings, prepare_ternary_staging_io,
...
pub fn filter_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_filter(OpMode::Normal, attrs, inputs, output)
}
...
fn dispatch_filter(
    ...
    crate::vk_trace!(
        "dispatch_filter mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 3 {
        return Err(anyhow!("filter expects 3 inputs, got {}", inputs.len()));
    }
    if inputs[0].shape() != inputs[1].shape() || inputs[0].shape() != inputs[2].shape() {
        return Err(anyhow!(
            "filter input shapes {:?}, {:?}, {:?} must match",
            inputs[0].shape(),
            inputs[1].shape(),
            inputs[2].shape()
        ));
    }
    let output = output.ok_or_else(|| anyhow!("filter requires an output tensor"))?;
    if output.shape() != inputs[0].shape() {
        return Err(anyhow!(
            "filter output shape {:?} does not match input shape {:?}",
            output.shape(),
            inputs[0].shape()
        ));
    }
    ...
    if output.dtype() != output_dtype {
        return Err(anyhow!(
            "filter requires output dtype {:?}, got {:?}",
            output_dtype,
            output.dtype()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::Filter, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::Filter, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan filter target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Filter.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<FilterPush>() as u32,
    };

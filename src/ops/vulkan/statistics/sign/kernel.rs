...
pub fn sign_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_sign(OpMode::Normal, attrs, inputs, output)
}
...
fn dispatch_sign(
    ...
    crate::vk_trace!(
        "dispatch_sign mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("sign expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("sign requires an output tensor"))?;
    let input_dtype = inputs[0].dtype();
    let output_dtype = DType::I8;
    if output.dtype() != DType::I8 {
        return Err(anyhow!(
            "sign requires i8 output dtype, got {:?}",
            output.dtype()
        ));
    }
    ...
    let target = match target_name(OpKind::Sign, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan sign target={}", target);

...
pub fn sum_axis_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
...
fn dispatch_sum_axis(
    ...
    crate::vk_trace!(
        "dispatch_sum_axis mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("sum_axis expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("sum_axis requires an output tensor"))?;
    ...
    if output.shape() != expected_shape.as_slice() {
        return Err(anyhow!(
            "sum_axis output shape {:?} does not match expected shape {:?}",
            output.shape(),
            expected_shape
        ));
    }
    ...
    if !op_supports_dtype(OpKind::SumAxis, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::SumAxis, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan sum_axis target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::SumAxis.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<SumAxisPush>() as u32,
    };

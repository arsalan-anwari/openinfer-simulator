...
pub fn argmax_axis_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_argmax_axis(OpMode::Normal, attrs, inputs, output)
}

fn dispatch_argmax_axis(
    mode: OpMode,
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
    ...
    crate::vk_trace!(
        "dispatch_argmax_axis mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 1 {
        return Err(anyhow!("argmax_axis expects 1 input, got {}", inputs.len()));
    }
    let output = output.ok_or_else(|| anyhow!("argmax_axis requires an output tensor"))?;
    let input_dtype = inputs[0].dtype();
    let output_dtype = DType::I64;
    if output.dtype() != DType::I64 {
        return Err(anyhow!(
            "argmax_axis requires i64 output dtype, got {:?}",
            output.dtype()
        ));
    }
    ...
    if output.shape() != expected_shape.as_slice() {
        return Err(anyhow!(
            "argmax_axis output shape {:?} does not match expected shape {:?}",
            output.shape(),
            expected_shape
        ));
    }
    ...
    if !op_supports_dtype(OpKind::ArgmaxAxis, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::ArgmaxAxis, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan argmax_axis target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::ArgmaxAxis.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<ArgmaxAxisPush>() as u32,
    };

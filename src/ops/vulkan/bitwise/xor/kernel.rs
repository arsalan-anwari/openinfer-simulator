...
pub fn xor_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_xor(OpMode::Normal, attrs, inputs, output)
}

pub fn xor_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_xor(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_xor(
    ...
    crate::vk_trace!(
        "dispatch_xor mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 2 {
        return Err(anyhow!("xor expects 2 inputs, got {}", inputs.len()));
    }
    if inputs[0].shape() != inputs[1].shape() {
        return Err(anyhow!(
            "xor input shapes {:?} and {:?} must match",
            inputs[0].shape(),
            inputs[1].shape()
        ));
    }
    let output = output.ok_or_else(|| anyhow!("xor requires an output tensor"))?;
    if output.shape() != inputs[0].shape() {
        return Err(anyhow!(
            "xor output shape {:?} does not match input shape {:?}",
            output.shape(),
            inputs[0].shape()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::Xor, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::Xor, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan xor target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Xor.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<XorPush>() as u32,
    };

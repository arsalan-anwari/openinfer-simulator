...
pub fn and_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_and(OpMode::Normal, attrs, inputs, output)
}
...
pub fn and_inplace_dispatch(
    ...
    dispatch_and(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_and(
    ...
    crate::vk_trace!(
        "dispatch_and mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 2 {
        return Err(anyhow!("and expects 2 inputs, got {}", inputs.len()));
    }
    if inputs[0].shape() != inputs[1].shape() {
        return Err(anyhow!(
            "and input shapes {:?} and {:?} must match",
            inputs[0].shape(),
            inputs[1].shape()
        ));
    }
    let output = output.ok_or_else(|| anyhow!("and requires an output tensor"))?;
    if output.shape() != inputs[0].shape() {
        return Err(anyhow!(
            "and output shape {:?} does not match input shape {:?}",
            output.shape(),
            inputs[0].shape()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::And, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::And, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan and target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::And.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<AndPush>() as u32,
    };

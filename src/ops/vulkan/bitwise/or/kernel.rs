...
pub fn or_normal_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_or(OpMode::Normal, attrs, inputs, output)
}

pub fn or_inplace_dispatch(
    attrs: &OpAttrs,
    inputs: &[TensorValue],
    output: Option<&mut TensorValue>,
) -> Result<()> {
    dispatch_or(OpMode::Inplace, attrs, inputs, output)
}
...
fn dispatch_or(
    ...
    crate::vk_trace!(
        "dispatch_or mode={:?} input_dtype={:?} output_dtype={:?}",
        mode,
        inputs.get(0).map(|t| t.dtype()),
        output.as_ref().map(|t| t.dtype())
    );
    if inputs.len() != 2 {
        return Err(anyhow!("or expects 2 inputs, got {}", inputs.len()));
    }
    if inputs[0].shape() != inputs[1].shape() {
        return Err(anyhow!(
            "or input shapes {:?} and {:?} must match",
            inputs[0].shape(),
            inputs[1].shape()
        ));
    }
    let output = output.ok_or_else(|| anyhow!("or requires an output tensor"))?;
    if output.shape() != inputs[0].shape() {
        return Err(anyhow!(
            "or output shape {:?} does not match input shape {:?}",
            output.shape(),
            inputs[0].shape()
        ));
    }
    ...
    if !op_supports_dtype(OpKind::Or, mode, input_dtype, output_dtype, attrs) {
        crate::vk_trace!(
    ...
    let target = match target_name(OpKind::Or, mode, input_dtype, output_dtype) {
        Ok(name) => name,
        Err(_) => {
    ...
    crate::vk_trace!("dispatching vulkan or target={}", target);
    let desc_bytes = build_desc_bytes(&descs);
    let push_bytes = bytemuck::bytes_of(&push).to_vec();
    let spec = VulkanOpSpec {
        entry: &target,
        spv_dir: crate::ops::vulkan::spv::spv_dir_for_op(OpKind::Or.as_str()),
        workgroup_size: [256, 1, 1],
        push_constant_size: std::mem::size_of::<OrPush>() as u32,
    };

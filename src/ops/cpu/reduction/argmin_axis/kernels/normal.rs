use anyhow::{anyhow, Result};

use crate::graph::OpAttrs;
use crate::ops::cpu::reduce::{
    axis_from_attrs, keepdims_from_attrs, linear_to_indices, output_offset, output_shape,
    output_strides, select_first_from_attrs,
};
use crate::tensor::{BF16, F16, F8, Tensor};

fn argmin_axis_f64<T: Copy>(
    attrs: &OpAttrs,
    a: &Tensor<T>,
    out: &mut Tensor<i64>,
    mut to_f64: impl FnMut(T) -> f64,
) -> Result<()> {
    let axis = axis_from_attrs(attrs, a.shape().len())?;
    let keepdims = keepdims_from_attrs(attrs);
    let select_first = select_first_from_attrs(attrs);
    let out_shape = output_shape(a.shape(), &[axis], keepdims);
    if out.shape() != out_shape.as_slice() {
        return Err(anyhow!(
            "output shape {:?} does not match expected shape {:?}",
            out.shape(),
            out_shape
        ));
    }
    let mut initialized = vec![false; out.data.len()];
    let mut best_vals = vec![0f64; out.data.len()];
    let mut best_idx = vec![0i64; out.data.len()];
    let out_strides = output_strides(&out_shape);
    for idx in 0..a.numel() {
        let coords = linear_to_indices(idx, a.shape());
        let out_offset = output_offset(&coords, &[axis], keepdims, &out_strides);
        let value = to_f64(a.data[idx]);
        let axis_idx = coords[axis] as i64;
        if !initialized[out_offset] {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
            initialized[out_offset] = true;
        } else if (select_first && value < best_vals[out_offset])
            || (!select_first && value <= best_vals[out_offset])
        {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
        }
    }
    for (slot, value) in out.data.iter_mut().zip(best_idx.into_iter()) {
        *slot = value;
    }
    Ok(())
}

fn argmin_axis_i64<T: Copy>(
    attrs: &OpAttrs,
    a: &Tensor<T>,
    out: &mut Tensor<i64>,
    mut to_i64: impl FnMut(T) -> i64,
) -> Result<()> {
    let axis = axis_from_attrs(attrs, a.shape().len())?;
    let keepdims = keepdims_from_attrs(attrs);
    let select_first = select_first_from_attrs(attrs);
    let out_shape = output_shape(a.shape(), &[axis], keepdims);
    if out.shape() != out_shape.as_slice() {
        return Err(anyhow!(
            "output shape {:?} does not match expected shape {:?}",
            out.shape(),
            out_shape
        ));
    }
    let mut initialized = vec![false; out.data.len()];
    let mut best_vals = vec![0i64; out.data.len()];
    let mut best_idx = vec![0i64; out.data.len()];
    let out_strides = output_strides(&out_shape);
    for idx in 0..a.numel() {
        let coords = linear_to_indices(idx, a.shape());
        let out_offset = output_offset(&coords, &[axis], keepdims, &out_strides);
        let value = to_i64(a.data[idx]);
        let axis_idx = coords[axis] as i64;
        if !initialized[out_offset] {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
            initialized[out_offset] = true;
        } else if (select_first && value < best_vals[out_offset])
            || (!select_first && value <= best_vals[out_offset])
        {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
        }
    }
    for (slot, value) in out.data.iter_mut().zip(best_idx.into_iter()) {
        *slot = value;
    }
    Ok(())
}

fn argmin_axis_u64<T: Copy>(
    attrs: &OpAttrs,
    a: &Tensor<T>,
    out: &mut Tensor<i64>,
    mut to_u64: impl FnMut(T) -> u64,
) -> Result<()> {
    let axis = axis_from_attrs(attrs, a.shape().len())?;
    let keepdims = keepdims_from_attrs(attrs);
    let select_first = select_first_from_attrs(attrs);
    let out_shape = output_shape(a.shape(), &[axis], keepdims);
    if out.shape() != out_shape.as_slice() {
        return Err(anyhow!(
            "output shape {:?} does not match expected shape {:?}",
            out.shape(),
            out_shape
        ));
    }
    let mut initialized = vec![false; out.data.len()];
    let mut best_vals = vec![0u64; out.data.len()];
    let mut best_idx = vec![0i64; out.data.len()];
    let out_strides = output_strides(&out_shape);
    for idx in 0..a.numel() {
        let coords = linear_to_indices(idx, a.shape());
        let out_offset = output_offset(&coords, &[axis], keepdims, &out_strides);
        let value = to_u64(a.data[idx]);
        let axis_idx = coords[axis] as i64;
        if !initialized[out_offset] {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
            initialized[out_offset] = true;
        } else if (select_first && value < best_vals[out_offset])
            || (!select_first && value <= best_vals[out_offset])
        {
            best_vals[out_offset] = value;
            best_idx[out_offset] = axis_idx;
        }
    }
    for (slot, value) in out.data.iter_mut().zip(best_idx.into_iter()) {
        *slot = value;
    }
    Ok(())
}

pub fn argmin_axis_f8_normal(attrs: &OpAttrs, a: &Tensor<F8>, out: &mut Tensor<i64>) -> Result<()> {
    argmin_axis_f64(attrs, a, out, |v| v.to_f32() as f64)
}

pub fn argmin_axis_bf16_normal(
    attrs: &OpAttrs,
    a: &Tensor<BF16>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_f64(attrs, a, out, |v| v.to_f32() as f64)
}

pub fn argmin_axis_f16_normal(
    attrs: &OpAttrs,
    a: &Tensor<F16>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_f64(attrs, a, out, |v| v.to_f32() as f64)
}

pub fn argmin_axis_f32_normal(
    attrs: &OpAttrs,
    a: &Tensor<f32>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_f64(attrs, a, out, |v| v as f64)
}

pub fn argmin_axis_f64_normal(
    attrs: &OpAttrs,
    a: &Tensor<f64>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_f64(attrs, a, out, |v| v)
}

pub fn argmin_axis_i8_normal(
    attrs: &OpAttrs,
    a: &Tensor<i8>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_i64(attrs, a, out, |v| v as i64)
}

pub fn argmin_axis_i16_normal(
    attrs: &OpAttrs,
    a: &Tensor<i16>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_i64(attrs, a, out, |v| v as i64)
}

pub fn argmin_axis_i32_normal(
    attrs: &OpAttrs,
    a: &Tensor<i32>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_i64(attrs, a, out, |v| v as i64)
}

pub fn argmin_axis_i64_normal(
    attrs: &OpAttrs,
    a: &Tensor<i64>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_i64(attrs, a, out, |v| v)
}

pub fn argmin_axis_u8_normal(
    attrs: &OpAttrs,
    a: &Tensor<u8>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_u64(attrs, a, out, |v| v as u64)
}

pub fn argmin_axis_u16_normal(
    attrs: &OpAttrs,
    a: &Tensor<u16>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_u64(attrs, a, out, |v| v as u64)
}

pub fn argmin_axis_u32_normal(
    attrs: &OpAttrs,
    a: &Tensor<u32>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_u64(attrs, a, out, |v| v as u64)
}

pub fn argmin_axis_u64_normal(
    attrs: &OpAttrs,
    a: &Tensor<u64>,
    out: &mut Tensor<i64>,
) -> Result<()> {
    argmin_axis_u64(attrs, a, out, |v| v)
}
